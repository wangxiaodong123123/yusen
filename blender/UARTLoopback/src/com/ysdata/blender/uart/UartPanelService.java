...
//      isStop = true;
    ...
    private void RequestComm() {
		...
		SendBuf[1] = (byte) Format.PANEL_BREAK;
	  	...
	  	SendBuf[3] = 0;
	  	SendBuf[4] = 4;
	  	SendBuf[5] = (byte) (start_seq_g >> 8);
	  	SendBuf[6] = (byte) (start_seq_g & 0xff);
	  	SendBuf[7] = (byte) (end_seq_g >> 8);
	  	SendBuf[8] = (byte) (end_seq_g & 0xff);
    ...
    private void RequestRecvPrjEngName() {
		...
		SendBuf[1] = (byte) Format.PANEL_BREAK;
	  	...
	  	SendBuf[2] = Format.CMD_MIX_REQUEST_ENG;
	  	SendBuf[3] = 0;
	  	SendBuf[4] = 0;
	  	...
	  	uartInterface.WriteData(SendBuf.length, SendBuf);
	  	SetCmd = Format.CMD_MIX_REQUEST_ENG;
    }
    ...
    private boolean WaitServerACK(int time_out) {
    	...
    	if (ServerCmd == SetCmd) {
    		...
    		switch (ServerCmd) {
    		case Format.CMD_MIX_REQUEST_ENG:
    			AppUtil.log( "WaitServerACK:Format.CMD_MIX_REQUEST_ENG");
    			break;
    		case Format.CMD_MIX_REQUEST_COMM_SEQ:
    			AppUtil.log( "WaitServerACK:Format.CMD_MIX_REQUEST_COMM_SEQ");
    			break;
    		}
    		SetCmd = 0xff;
    		ServerCmd = 0xff;
...
//					data_len = recv_length - 6;
					...
					if (!requestConn) {						//判断是否已应答控制面板联网请求
						if (cmd == Format.CMD_MIX_REQUEST_INTERNET) {
							AppUtil.log( "============CMD_MIX_REQUEST_INTERNET=============");
							int mix_counts = 0;
							int index = 5 + cut_recv_index;
							if (data_len == 2) {
								AppUtil.log("index="+index);
								for (int i = 0; i < 9; i++) {
									AppUtil.log(Integer.toHexString(recv_data[i]).toString());
								}
								mix_counts = (recv_data[index++] << 8) + recv_data[index++];
								AppUtil.log("mix_counts:"+mix_counts);
						    	mHandler.sendMessage(mHandler.obtainMessage(Format.HANDLER_REQUEST_INTERNET, 
						    			mix_counts, 0));
							} else {
								AppUtil.log("requestConn : data_len error---"+data_len);
								break;
							}
						...
						if (!requestPrjEng) {
							if (cmd == Format.CMD_MIX_REQUEST_ENG) {
								if (!recv_timeout) { //add by wxd 20150902
									AppUtil.log( "============CMD_MIX_REQUEST_ENG=============");
									ServerCmd = Format.CMD_MIX_REQUEST_ENG; //接收控制面板主工程名称与子工程名称
									for (int i = 0; i < 100; i++) {
										try {
									...
									try {
										...
										if (eng_name.equals(eng_name_g)) {
											...
											if (mProjectBase.openDb()) {
												...
												mHandler.sendEmptyMessage(Format.HANDLER_ENG_MATCH);
											} else {
											...
											mHandler.sendEmptyMessage(Format.HANDLER_ENG_NOT_MATCH);
										}
						...
						if (!requestComm) {
							if (cmd == Format.CMD_MIX_REQUEST_COMM_SEQ) {
								if (!recv_timeout) { //add by wxd 20150902
									AppUtil.log( "============CMD_MIX_REQUEST_COMM_SEQ=============");
									ServerCmd = Format.CMD_MIX_REQUEST_COMM_SEQ; //接收控制面板发数据应答
									for (int i = 0; i < 100; i++) {
										try {
									...
									mHandler.sendEmptyMessage(Format.HANDLER_REQEST_TRANSFER_DATA_SUCCESS);
								}
							} 
						} else {
							if (cmd == Format.CMD_MIX_DATA) {
								AppUtil.log("======CMD_MIX_DATA=======");
								if (data_len > 0) {
									int index = 5 + cut_recv_index;
									try {
										//搅拌序号
										mix_id_g = (recv_data[index++] << 8) + recv_data[index++];
										AppUtil.log("mix_id_g:"+mix_id_g);
										//搅拌日期
										...
										//开始时间，结束时间
										int start_time_h = recv_data[index++];
										int start_time_m = recv_data[index++];
										
										StringBuffer start_time = new StringBuffer();
										if (start_time_h < 10) {
											start_time.append(0);
										}
										start_time.append(start_time_h).append(':');
										if (start_time_m < 10) {
											start_time.append(0);
										}
										start_time.append(start_time_m);
										AppUtil.log("start_time:"+start_time.toString());
										
										int end_time_h = recv_data[index++];
										int end_time_m = recv_data[index++];
										
										StringBuffer end_time = new StringBuffer();
										if (end_time_h < 10) {
											end_time.append(0);
										}
										end_time.append(end_time_h).append(':');
										if (end_time_m < 10) {
											end_time.append(0);
										}
										end_time.append(end_time_m);
										AppUtil.log("end_time:"+end_time.toString());
										
										//水灰比
										byte[] mix_ratio_arr = new byte[64];
										int mix_ratio_len = recv_data[index++];
										mix_ratio_len = 4;
										for (int i = 0; i < mix_ratio_len; i++) {
											mix_ratio_arr[i] = (byte) recv_data[index++];
										}
										String mix_ratio_string = "";
										mix_ratio_string = new String(mix_ratio_arr, 0, mix_ratio_len, "GBK");
										
										double mix_ratio = Double.parseDouble(mix_ratio_string);
										AppUtil.log("mix_ratio:"+mix_ratio);
										int weight = (recv_data[index++] << 8) + recv_data[index++]; //千克
										int position = (recv_data[index++] << 8) + recv_data[index++];
										AppUtil.log("weight:"+weight);
										AppUtil.log("position:"+position);
										
										sendAck(cmd); //接收到正确数据，发送数据接收应答
										mHandler.sendMessage(mHandler.obtainMessage(Format.HANDLER_RECV_MIX_DATE,
												new MixCraftParameter(mix_id_g, mix_ratio, date.toString(), start_time.toString(), 
														end_time.toString(), weight, position)));
										
										if (mProjectPointBase.openDb(proj_val, eng_val)) {
											mProjectPointBase.insertMixCollectRecord(mix_id_g, mix_ratio, date.toString(),
													start_time.toString(), end_time.toString(), weight, position);
										} else {
											Toast.makeText(context, "数据库打开失败", Toast.LENGTH_SHORT).show();
										}
									} catch (Exception e) {
							...
							} else if (cmd == Format.CMD_MIX_RESULT_PIC) {
								AppUtil.log("======CMD_MIX_RESULT_PIC=======");
								if (data_len > 0) {
									...
									try {
										...
										mHandler.sendMessage(mHandler.obtainMessage(Format.HANDLER_RECV_GROUP_MIX_PIC_RESULT,
												pic_total_size, pic_size_index));
										...
										if (pic_size_index == pic_total_size) {
 											if (mProjectPointBase.openDb(proj_val, eng_val)) {
 												mProjectPointBase.updateMixCollectPicResult(mix_id_g, pic_data_sb.toString());
 											} else {
 												Toast.makeText(context, "数据库打开失败", Toast.LENGTH_SHORT).show();
 											}
 											pic_size_index = 0;
							...
							} else if (cmd == Format.CMD_MIX_SCENE_PIC) {
								AppUtil.log("======CMD_MIX_SCENE_PIC=======");
								if (data_len > 0) {
									/*for (int i = 0; i < data_len+6; i++) {
										AppUtil.log( "recv_data["+i+"]:" + recv_data[i]);
									}*/
									...
									try {
										...
										pic_size_index += pic_group_size;
										AppUtil.log("pic_total_size:"+pic_total_size+" pic_group_id:"+pic_group_id + " pic_size_index:"+pic_size_index);
										mHandler.sendMessage(mHandler.obtainMessage(Format.HANDLER_RECV_GROUP_MIX_PIC_SCENE,
												pic_total_size, pic_size_index));
										if (pic_size_index <= pic_total_size) {
											for (int i = 0; i < pic_group_size; i++) {
												pic_data_sb.append(recv_data[index++]).append(' ');
										...
										if (pic_size_index == pic_total_size) {
 											if (mProjectPointBase.openDb(proj_val, eng_val)) {
 												mProjectPointBase.updateMixCollectPicScene(mix_id_g, pic_data_sb.toString());
 											} else {
 												Toast.makeText(context, "数据库打开失败", Toast.LENGTH_SHORT).show();
 											}
 											pic_size_index = 0;
							...
							} else if (cmd == Format.CMD_MIX_COMM_STOP) {
								AppUtil.log( "recv CMD_RECEV_STOP");
								if (mProjectPointBase.openDb(proj_val, eng_val)) {
									mProjectPointBase.setCollectOrderNo();
								}
								sendAck(cmd); //接收到正确数据，发送数据接收应答
								mHandler.sendEmptyMessage(Format.HANDLER_MIX_COMM_FINISH);
								try {
									Thread.sleep(2000);
								} catch (InterruptedException e) {
									e.printStackTrace();
