...
public class ProjectDataBaseAdapter {
	...
	private static final String SUBPROJECT_LIST_START_MILEAGE = "start_mileage";
	...
	private static final int VOLUME_SUBPROJECT_LIST_START_MILEAGE = 4;
	...
	private static final String DATABASE_CREATE_SUBPROJECT_LIST_TABLE = "CREATE TABLE IF NOT EXISTS " + SUBPROJECT_LIST_TABLE + "("
			...
			+ SUBPROJECT_LIST_START_MILEAGE + " VARCHAR(40)"
			+ ");";
    ...
    public void saveSubProectRecord(int subproject_id, int project_id, String name, int direction, String start_mileage) {
    	...
    	mDb.execSQL("REPLACE INTO " + SUBPROJECT_LIST_TABLE + " ("
    			...
    			+ SUBPROJCET_LIST_DIRECTION + ","
    			+ SUBPROJECT_LIST_START_MILEAGE 
    			+ ") VALUES ("
    			+ subproject_id + "," + project_id + ", ?, " + direction
    			+ ", ?)",new Object[]{name, start_mileage}); 
    }
    
    public String getSubProjectStartMileage(int subproject_id) {
		...
		if (cursor.moveToNext()) {
			start_mileage = cursor.getString(VOLUME_SUBPROJECT_LIST_START_MILEAGE);
    	}
    	if (cursor != null) {
    		cursor.close();
    	}
