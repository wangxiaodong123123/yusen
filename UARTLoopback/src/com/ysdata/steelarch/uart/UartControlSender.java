...
public class UartControlSender {
    ...
    private SteelArchCraftParameter mSteelArchCraftParameter;	

    int steelarch_start_orderno;
    int steelarch_end_orderno;
    	...
    	public void onReceive(Context context, Intent intent) {
    		...
    		if (action.equals(Format.SEND_CONTROL_DATA)) {
    			...
    			if (mProjectPointBase.openDb(project_id, subproject_id)) {
	    			mSteelArchCraftParameter = mProjectPointBase.getSteelArchCraftParameterWhereOrderNo(steelarch_start_orderno);
    				if (mSteelArchCraftParameter == null) {
    					Toast.makeText(context, "数据库提取数据失败", Toast.LENGTH_SHORT).show();
    				} else {
    					steelarchName = mSteelArchCraftParameter.getName();
...
//        setState(BTMESSAGE.STATE_LISTEN);
    ...
    private void sendConnectAck() {
		...
		SendBuf[1] = (byte) Format.PANEL_BREAK;
	  	...
	  	SendBuf[2] = Format.CMD_PANEL_REQUEST_INTERNET; //控制面板联网请求;
	  	SendBuf[3] = 0;
	  	SendBuf[4] = 0;
	  	check_sum = getCrcCheckSum(5, SendBuf);
	  	SendBuf[5] = (byte) ((check_sum >> 8) & 0xff);
    ...
    private void sendSteelArchCraftParameter(int orderno) {

    	mSteelArchCraftParameter = mProjectPointBase.getSteelArchCraftParameterWhereOrderNo(orderno);
    	
       String name = mSteelArchCraftParameter.getName();
       double design_distance =    mSteelArchCraftParameter.getSteelarchToSteelarchDistance();
		...
		index = 5;
		SendBuf[index++] = (byte) (orderno >> 16);
		SendBuf[index++] = (byte) (orderno >> 8);
		SendBuf[index++] = (byte) (orderno & 0xff);
		
		try {
			byte[] name_byte = name.getBytes("GBK");
			int length = name_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = name_byte[i];
			}
			
			byte[] designDistance_byte = Double.toString(design_distance).getBytes("GBK");
			length = designDistance_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = designDistance_byte[i];
			}
			SendBuf[3] = (byte) ((index - 5) >> 8);
			SendBuf[4] = (byte) ((index - 5) & 0xff);
		...
		AppUtil.log( "------sendSteelArchCraftParameter------orderno:"+orderno + 
			" name:"+ name + " design_distance:" + design_distance);
		uartInterface.WriteData(index+2, SendBuf);
    }	
								...
								while (ReSendCounter > 0) {
									...
									if (isSystemInfo) {
										...
										if (steelarch_start_orderno <= steelarch_end_orderno) {
											...
											if (mProjectPointBase.openDb(project_id, subproject_id)) {
												sendSteelArchCraftParameter(steelarch_start_orderno);
									...
									if (WaitServerACK(10000) && (ACK == 1)) {
										...
										if (ReSendCounter <= 0) {
											...
											if (SetCmd == Format.CMD_CRAFT_PARAMETER) {
												context.sendBroadcast(new Intent(Format.RECEIVE_STEELARCH_PARAM_ACK_TIMEOUT));
											}
											connectionFailed();
											break;
										}
						...
						if (!recv_timeout) {  //在数据发送超时后与下一次数据发送前接收到的应答不做处理，防止连续处理多个应答信号
							if (cmd == Format.CMD_SYSTEM_INFO) {
								...
								if (recv_data[cut_recv_index+5] == 1) {
									...
									if (mProjectPointBase.openDb(project_id, subproject_id)) {
										mProjectPointBase.updateCraftTransferState(steelarch_start_orderno, 1); //add 20150827 接收到应答后标识置筋参数成功传输状态
