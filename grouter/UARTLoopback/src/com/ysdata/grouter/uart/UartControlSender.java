...
public class UartControlSender {
	...
	private int g_prev_anchor_id = 0;
	private String xing = "";
	int ACK = 0;
    ...
    private int mState;
    private MileageParameter mMileageParameter;
    private ArrayList<AnchorParameter> anchor_list;
    	...
    	public void onReceive(Context context, Intent intent) {
    		...
    		if (action.equals(Format.SEND_CONTROL_DATA)) {
    			...
    			AppUtil.log("anchor_start_orderno:"+anchor_start_orderno + " anchor_end_orderno:"+anchor_end_orderno
    					+" mileage_start_page:"+mileage_start_orderno + " mileage_end_orderno:"+mileage_end_orderno);
    			if (mProjectPointBase.openDb(project_id, subproject_id)) {
	    			mMileageParameter = mProjectPointBase.getMileageParameter(mileage_start_orderno);
    				anchor_list = mProjectPointBase.queryCraftParamDevID(mileage_start_orderno); 
    				if (mMileageParameter == null || anchor_list == null) {
    					Toast.makeText(context, "数据库提取数据失败", Toast.LENGTH_SHORT).show();
    				} else {
    					mileage_anchor_start_orderno = anchor_start_orderno;
    					mileage_anchor_end_orderno = anchor_start_orderno + anchor_list.size() - 1;
    					mileage_string = mMileageParameter.getMileageName();
    					eng_name_g = intent.getStringExtra("eng_name");
    					proj_name_g = intent.getStringExtra("proj_name");
    					mileage_start_orderno++;
    		...
    		} else if (action.equals(Format.SEND_CONTROL_DATA)) {
				stop();
			}
    	};
    };
...
//        setState(BTMESSAGE.STATE_LISTEN);
    ...
    private void sendSystemInfo() {
    	...
    	String proj_name = proj_name_g;
    	
    	if (eng_name.length() > 1) {
    		xing = eng_name.substring(0, 2);
    		if (!xing.equals("上行") && !xing.equals("下行")) {
    			xing = "";
    		}
    	} else {
    		xing = "";
		...
		SendBuf[2] = Format.CMD_SYSTEM_INFO;
		AppUtil.log( "------sendSystemInfo------");
		index = 5;
		...
		try {
			...
			byte[] projName_byte = proj_name.getBytes("GBK");
			length = projName_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = projName_byte[i];
			}
    ...
    private void sendAnchorParameter(int anchor_id) {
    	if (anchor_id >= mileage_anchor_start_orderno && anchor_id <= mileage_anchor_end_orderno) {
    		if (g_prev_anchor_id > 0) {
    			anchor_seq += anchor_id - g_prev_anchor_id; 
    		} else {
    			anchor_seq = 0;
    		}
    	} else {
			mMileageParameter = mProjectPointBase.getMileageParameter(mileage_start_orderno);
			anchor_list = mProjectPointBase.queryCraftParamDevID(mileage_start_orderno);
			mileage_anchor_start_orderno = anchor_id;
			mileage_anchor_end_orderno = anchor_id + anchor_list.size() - 1;
			mileage_string = mMileageParameter.getMileageName();
			anchor_seq = 0;
			mileage_start_orderno++;
    	}
    	g_prev_anchor_id = anchor_id;
    	AnchorParameter arch_param = anchor_list.get(anchor_seq);
//    	String seq = xing+mileage_string+"-"+arch_param.getSeq();
    	String seq = arch_param.getAnchorName();
    	int grout_priority = arch_param.getGroutingPriority();
    	String mileage = mileage_string;
    	double cap_unit_hour = arch_param.getUnitHourCap();
    	String anchor_type = arch_param.getAnchorType();
    	String anchor_model = arch_param.getAnchorModel();
    	double anchor_length = arch_param.getAnchorLen();
    	double thereo_cap = arch_param.getDesignCap();
    	String device_type = "智能注浆泵";
    	double design_press = arch_param.getAnchorDesignPressure();
    	int hold_time = arch_param.getHoldTime();
    	double full_hole_pressure = arch_param.getFullHolePressure();
//    	int anchor_seq = anchor_id;
//    	String eng_name = eng_name_g;
//    	String proj_name = proj_name_g;
    	int index = 0;
    	Intent intent = new Intent(Format.SENDING_PARAM);
    	intent.putExtra("seq", seq);
    	context.sendBroadcast(intent);
    	byte[] SendBuf = new byte[1024];
    	SendBuf[0] = (byte) Format.PANEL_HEAD;
		SendBuf[1] = Format.PANEL_BREAK;
		SendBuf[2] = Format.CMD_CRAFT_PARAMETER;
		index = 5;
//		AppUtil.log( "anchor_id:"+anchor_id);
		SendBuf[index++] = (byte) (anchor_id >> 16);
		SendBuf[index++] = (byte) (anchor_id >> 8);
		SendBuf[index++] = (byte) (anchor_id & 0xff);
		SendBuf[index++] = (byte) grout_priority;
		try {
			byte[] seq_byte = seq.getBytes("GBK");
			int length = seq_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = seq_byte[i];
			}
			byte[] mileage_byte = mileage.getBytes("GBK");
			length = mileage_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = mileage_byte[i];
			}
			byte[] capUnitHour_byte = Double.toString(cap_unit_hour).getBytes("GBK");
			length = capUnitHour_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = capUnitHour_byte[i];
			}
			String anchor_type_model = anchor_type+"-"+anchor_model;
			byte[] anchorTypeModel_byte = anchor_type_model.getBytes("GBK");
			length = anchorTypeModel_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = anchorTypeModel_byte[i];
			}
			byte[] anchorLength_byte = Double.toString(anchor_length).getBytes("GBK");
			length = anchorLength_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = anchorLength_byte[i];
			}
			byte[] thereoCap_byte = Double.toString(thereo_cap).getBytes("GBK");
			length = thereoCap_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = thereoCap_byte[i];
			}
			byte[] deviceType_byte = device_type.getBytes("GBK");
			length = deviceType_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = deviceType_byte[i];
			}
			byte[] designPress_byte = Double.toString(design_press).getBytes("GBK");
			length = designPress_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = designPress_byte[i];
			}
			byte[] holdTime_byte = Integer.toString(hold_time).getBytes("GBK");
			length = holdTime_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = holdTime_byte[i];
			}
			byte[] fullHolePressure_byte = Double.toString(full_hole_pressure).getBytes("GBK");
			length = fullHolePressure_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = fullHolePressure_byte[i];
			}
			/*byte[] engName_byte = eng_name.getBytes("GBK");
			length = engName_byte.length;
			SendBuf[index++] = (byte) length;
			for (int i = 0; i < length; i++) {
				SendBuf[index++] = engName_byte[i];
...
//				check_sum += SendBuf[i];
//			}
//			SendBuf[index] = (byte) (check_sum & 0xff);
			int check_sum = getCrcCheckSum(index, SendBuf);
		...
		AppUtil.log( "------sendAnchorParameter------anchor_id:"+anchor_id + " anchor_name:"+
				arch_param.getAnchorName());
		uartInterface.WriteData(index+2, SendBuf);
    }
...
//										uartPrint((byte) 2);
										sendSystemInfo();
										SetCmd = Format.CMD_SYSTEM_INFO;
									} else {
										if (anchor_start_orderno <= anchor_end_orderno) {
											try {
												Thread.sleep(500); //为了控制面板可以看到每根置筋的参数，顾加了500ms延时，实际应用中可以考虑注释掉
											} catch (InterruptedException e) {
												e.printStackTrace();
											}
											if (mProjectPointBase.openDb(project_id, subproject_id)) {
												sendAnchorParameter(anchor_start_orderno);
												SetCmd = Format.CMD_CRAFT_PARAMETER;
											} else {
												Toast.makeText(context, "数据库打开失败", Toast.LENGTH_SHORT).show();
											}
										} else {
									...
									if (WaitServerACK(10000) && (ACK == 1)) {
										...
										if (ReSendCounter <= 0) {
											...
											if (SetCmd == Format.CMD_SYSTEM_INFO) {
												context.sendBroadcast(new Intent(Format.RECEIVE_SYSTEM_INFO_ACK_TIMEOUT));
											}
											if (SetCmd == Format.CMD_CRAFT_PARAMETER) {
												context.sendBroadcast(new Intent(Format.RECEIVE_ANCHOR_PARAM_ACK_TIMEOUT));
											}
											connectionFailed();
//											context.sendBroadcast(new Intent(Format.RECEIVE_DATA_TIMEOUT));
											break;
										}
									}
								}
...
//						AppUtil.log( Integer.toHexString(recv_data[i]).toString());
					...
					if (!requestConn) {
						if (cmd == Format.CMD_PANEL_REQUEST_INTERNET) { //接收控制面板联网到平板请求
							if (ACK != 1) ACK = 1;
...
//							uartPrint((byte) 1);
						...
						if (!recv_timeout) {  //在数据发送超时后与下一次数据发送前接收到的应答不做处理，防止连续处理多个应答信号
							if (cmd == Format.CMD_SYSTEM_INFO) {
								AppUtil.log( "recv CMD_SYSTEM_INFO");
								if (recv_data[cut_recv_index+5] == 1) {
									AppUtil.log( "pad send system info to panel success.");
									context.sendBroadcast(new Intent(Format.SUCCESS_SEND_SYSTEM_INFO));
									isSend = true;
									sendStart = true;
									isSystemInfo = false;  //发送的系统信息，如果校验通过才能发送置筋数据，否则重发 add by wxd 20150912
									ACK = 1;
								} else {
									AppUtil.log( "pad send system info to panel failed.");
									context.sendBroadcast(new Intent(Format.FAILED_SEND_SYSTEM_INFO));
//								isSend = true; //如果接收到NACK，程序继续接收面板数据  comment by wxd 20150912
									ACK = 0;
								}
//								isSystemInfo = false;  //comment by wxd 20150912
								ServerCmd = Format.CMD_SYSTEM_INFO;
								try {
									Thread.sleep(1000);
								} catch (InterruptedException e) {
									e.printStackTrace();
								}
							}	else if (cmd == Format.CMD_CRAFT_PARAMETER) {
								if (recv_data[cut_recv_index+5] == 1) {
									AppUtil.log( "pad set data to panel success.");
									if (mProjectPointBase.openDb(project_id, subproject_id)) {
										mProjectPointBase.updateCraftTransferState(anchor_start_orderno, 1); //add 20150827 接收到应答后标识置筋参数成功传输状态
										anchor_start_orderno++;
										context.sendBroadcast(new Intent(Format.SUCCESS_SEND_DATA));
										isSend = true;
										sendStart = true;
										ACK = 1;
									} else {
									...
									AppUtil.log( "pad set data to panel failed.");
									context.sendBroadcast(new Intent(Format.FAILED_SEND_DATA));
//								isSend = true; //如果接收到NACK，程序继续接收面板数据 comment by wxd 20150912
									ACK = 0;
								}
								AppUtil.log( "recv CMD_CRAFT_PARAMETER");
								ServerCmd = Format.CMD_CRAFT_PARAMETER;
							} else if (cmd == Format.CMD_COMM_STOP) {
								ServerCmd = Format.CMD_COMM_STOP;
								requestConn = false;
								isSend = false;
								sendStart = false;
								g_prev_anchor_id = 0;
								ACK = 1;
//							globleInit();
								context.sendBroadcast(new Intent(Format.SEND_DATA_FINISH));
								break;
							}
						}
					}
